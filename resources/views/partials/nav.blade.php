...
<nav class="site-nav sticky top-0 z-50">
    <div class="site-nav__pattern-strip" aria-hidden="true"></div>
    ...
    <div class="site-nav__body">
        <div class="mx-auto flex max-w-6xl flex-wrap items-center justify-between gap-3 px-4 py-3 sm:px-6 sm:py-4">
            <a href="{{ route('home') }}" class="site-nav__logo shrink-0 transition opacity-95 hover:opacity-100">
                <x-logo variant="dark" size="sm" :align="app()->getLocale() === 'ar' ? 'end' : 'start'" />
            </a>
            ...
            <div class="flex items-center gap-2 md:order-3">
                ...
                <button type="button" id="nav-toggle" class="site-nav__toggle rounded-lg p-2 text-ocean-deep hover:bg-white/70 md:hidden" aria-label="{{ __('site.nav.toggle_menu') }}" aria-expanded="false">
            ...
            <div id="nav-menu" class="site-nav__menu hidden w-full flex-col gap-1 border-t border-ocean-deep/10 pt-4 md:order-2 md:flex md:w-auto md:flex-1 md:flex-row md:items-center md:justify-end md:gap-1.5 md:border-0 md:pt-0 lg:gap-2">
    ...
    <div class="site-nav__pattern-border" aria-hidden="true"></div>
</nav>
